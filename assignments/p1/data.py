...
# We truncate the test set to 1k examples to expedite evaluation.
test_size = 1000


def load_imdb():
	train_dataset = load_dataset('imdb', split = 'train')
	dataset = train_dataset.train_test_split(test_size = test_size)
	train_dataset, val_dataset = dataset['train'], dataset['test']
	test_dataset = load_dataset('imdb', split = 'test')
	test_dataset = test_dataset.shuffle(seed = seed)
	test_dataset = test_dataset.select(list(range(test_size)))
